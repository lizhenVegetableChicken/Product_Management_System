...
from PyQt5.QtWidgets import QWidget, QApplication, QTableView, QHeaderView, QDialog

from OutNotInput import OutNotInput
from UI.MySearchBatchModelPIM import MySearchTableModelPIM, CheckBoxHeader
from UI.SearchView import MySearchWidget
from Utils import openDB

class OutInputDBTimes(MySearchWidget):
    def __init__(self):
        super(OutInputDBTimes, self).__init__()
        self.select_conditions = ["InDate", "ProductID"]
    ...
    def setupUi(self, Form):
        ...
        # hsj 自动义的tableModel
        slt = "ProductID,OutDate,InDate,T_In_Base.CreateID,InTechState"
        headerRow = ["产品编号", "出库时间", "入库时间", "归还人", "入库技术状态"]
        self.queryModel = MySearchTableModelPIM("T_Out_Base,T_In_Detail,T_In_Base", headerRow, slt)
    ...
    def retranslateUi(self, Form):
        ...
        self.label.setText(_translate("Form", "产品出入库"))
        self.label_3.setText(_translate("Form", "时间段"))
        self.label_4.setText(_translate("Form", "——"))
        self.label_5.setText(_translate("Form", "产品编号"))
        self.pushButton_2.setText(_translate("Form", "查询"))
        self.pushButton_4.setText(_translate("Form", "出库未入库产品"))
    ...
    def bindButton(self):
        ...
        self.pushButton_4.clicked.connect(self.ButtonEventD)

    def ButtonEventD(self):
        widget = OutNotInput()
        dialog = QDialog()
        ...
        if content == "":
            ...
            if len(content) == 2:
                if content[0] == content[1]:
                    sql = "WHERE T_Out_Base.OutNO=T_In_Base.OutNO AND T_In_Detail.ID=T_In_Base.ID"
                else:
                    sql = "WHERE T_Out_Base.OutNO=T_In_Base.OutNO AND T_In_Detail.ID=T_In_Base.ID and %s >= '%s' and %s <= '%s'" % (
                        self.select_conditions[0], content[0], self.select_conditions[0], content[1])
            elif len(content) == 3:
                if content[0] == content[1]:
                    sql = "WHERE T_Out_Base.OutNO=T_In_Base.OutNO AND T_In_Detail.ID=T_In_Base.ID and %s = '%s'"% (self.select_conditions[1], content[2])
                else:
                    sql = "WHERE T_Out_Base.OutNO=T_In_Base.OutNO AND T_In_Detail.ID=T_In_Base.ID and %s >= '%s' and %s <= '%s' and %s = '%s'" % (
                        self.select_conditions[0], content[0], self.select_conditions[0], content[1],
