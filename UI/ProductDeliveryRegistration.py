...
class ProductDeliveryRegistration(MySearchWidget):
    def __init__(self):
        ...
        self.select_conditions = ["ReceiverDate", "ProductNO"]
    ...
    def setupUi(self, Form):
        ...
        slt = "ProductID,ProductNO,ProductName,InitCount"
        headerRow = ["产品编号", "产品代号","产品名称", "交付数量", "合计(同一产品代号)"]
    ...
    def retranslateUi(self, Form):
        ...
        self.label_3.setText(_translate("Form", "登记起始日期"))
        self.label_4.setText(_translate("Form", "登记终止日期"))
        self.label_5.setText(_translate("Form", "产品代号"))
        ...
        if content == "":
            ...
            if len(content) == 2:
                if content[0] == content[1]:
                    ...
                    sql = "WHERE %s >= '%s' and %s <= '%s' and" % (
                    self.select_conditions[0], content[0], self.select_conditions[0], content[1])
            elif len(content) == 3:
                if content[0] == content[1]:
                    sql = "WHERE %s = '%s' and"% (self.select_conditions[1], content[2])
                else:
                    sql = "WHERE %s >= '%s' and %s <= '%s' and %s = '%s' and" % (
                        self.select_conditions[0], content[0], self.select_conditions[0], content[1],
                        self.select_conditions[1], content[2])

            self.queryModel.searchTableD(sql)
            self.updateUI()
